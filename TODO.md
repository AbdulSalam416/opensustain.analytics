...
Seven of the nine Streamlit tabs are done. The two tab gaps below (A, C)
are both **unblocked on data** — every payload the missing work needs is
...
Currently two truncated 8-item lists (top countries, organisation types).
Data is all in `organizations-overview.json`.
...
- [ ] Carry the same two filters onto Projects by Organisation and the
      Organisations tab charts (A) once those land
